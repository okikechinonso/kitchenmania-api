...
func (pg *PostgresDB) TokenInBlacklist(token *string) bool {
	result := pg.PostgresClient.Where("token = ?", *token).Find(&entity.Blacklist{})
	return result.Error != nil

}

func (d *PostgresDB) AddToBlackList(blacklist *entity.Blacklist) error {
	result := d.PostgresClient.Create(blacklist)
	return result.Error
}
